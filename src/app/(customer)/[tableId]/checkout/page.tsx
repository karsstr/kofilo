...
// =============================================================
// Page: /{tableId}/checkout
// Halaman Checkout PWA
// PERUBAHAN: Menambahkan logika pengiriman API ke /api/v1/pwa/orders
// =============================================================
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      const items = cart.map((item) => ({
        productId: item.id.split('-')[0], // ambil productId asli (sebelum suffix varian)
        name: item.name,
        price: item.price,
      ...
      const res = await fetch('/api/v1/pwa/orders', {
        ...
        body: JSON.stringify({ tableId, items, paymentMethod }),
      });
